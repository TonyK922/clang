...
- 内存分配和回收方式不同
  - 栈由操作系统自动分配和释放, 更快
  - 堆一般由程序员自己分配和释放, 速度较慢, 且容易产生碎片. 若程序员忘了释放, 程序结束可能由OS回收.
- 存放的内容不同
  - 栈一般存放函数参数, 与局部变量
  - 堆存放内容没有限制, 由程序员决定.
- 大小限制不同
  - 栈是由高地址向低地址生长的一片连续内存区, 栈顶地址和栈的容量, 由系统预先定义好.
  - 堆是由低地址向高地址生长, 区域可以不连续, 空间大, 使用比较灵活. 一般由内存管理来管理这块内存区.
- 数据结构不同
  - 栈 后进先出
  - 堆, 类似链表 整个内存结构可以看做树.
...
为了满足这种需求，C 语言提供了一种不定类型的指针，叫做 void 指针。它只有内存块的地址信息，没有类型信息，等到使用该块内存的时候，再向编译器补充说明，里面的数据类型是什么。void指针也叫通用指针.
...
注意，**`malloc()`不会对所分配的内存进行初始化，里面还保存着原来的值。**如果没有初始化，就使用这段内存，可能从里面读到以前的值。程序员要自己负责初始化，比如，字符串初始化可以使用`strcpy()`函数。其他类型可以用`memset(*dest, val, size_t);` 这个函数初始化.
...
**`free`函数不返回值, `free`掉指针指向的内存地址之后, 不对该指针的值做任何操作. 所以, 安全起见, 程序员应该把该指针手动赋值为`NULL`.**
...
**`realloc()`的第一个参数可以是 `NULL`，这时就相当于malloc**。
...
**如果`realloc()`的第二个参数是`0`，就会释放掉内存块**。**此时指针会指向NULL.**
...
**这个说明符只是告诉编译器, 你就这么给我优化就好了, 代码这边, 由我程序员来确保不会用别的指针来访问这段内存.**
...
- 使用完后, 忘记释放内存, 造成内存泄露.
  ...
  这块内存不用了, 也不释放, 然后指针指向下一块内存开始继续操作. 时间一长都忘了这个地方还有块内存没释放. 如此不断重蹈覆辙.

  如何防范?

  - 申请的内存, 使用完必须立刻释放.
  - 要保存申请的内存空间的首地址.
  - 防止内存越界, 可能会破坏其他动态内存, 造成free失败.
  - 多人开发, 谁申请谁释放.
  - 申请后确认申请成功再用.
  - 分配空间可以多申请点, 防止踩别人
  - 函数错误 return退出之前, 先释放需要释放的空间.

- 野指针

  野指针, 就是指向不明确的指针. 比如声明了指针变量, 不初始化, 就去解引用了.

  如何避免:

  - 内存被free后, 指针紧接着就P = NULL;

    ```c
    void myfree(void **ptr) //比如你直接重新封装个free函数, free的同时就给指针赋值NULL.
    {
        free(*ptr);
        *ptr = NULL;
    }
