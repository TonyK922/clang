...
基本数据类型有三种：**字符**（char）、整数（int）和浮点数（float）。(整型和浮点型合成**数值类型**)

其他的均属于 构造类型, 构造类型皆由上面的基本类型组成.

C99新增的数据类型:

_Bool , long long , _Complex(float, double, long double), _Imaginary(float, double, long double)
...
在计算机内部，字符类型使用一个字节（8位）存储。C 语言将其当作整数处理，所以字符类型就是宽度为一个字节的整数。每个字符对应一个整数（由 **ASCII 码**确定），比如`B`对应整数`66`。

> 标准ASCII码, 是用低7位表示字符的值, 最高位是奇偶校验位
...
printf函数无论是%d 还是%c 输出, 参数都是int类型.
...
- **注意**:
  ...
  **char在运算时, 会先隐式提升到int**, 运算完再把高位截断, 留一个字节.

  ```c
  char x = 'a';
  char y = 2;
  char sum = x + y;  //x和y都先提升到int 结果 再截断.
  ```
...
**通常, 程序中使用的整型数字, 都被存储为int类型. 如果数字范围大于int, 则编译器会自动类型转换.**

因为int型被认为是计算机处理整数类型时最高效的类型.
...
- `short int`（简写为`short`）：占用空间**不多于**`int`，一般占用2个字节（整数范围为-32768～32767)。
- `long int`（简写为`long`）：占用空间**不少于**`int`，至少为4个字节。
- `long long int`（简写为`long long`）：占用空间**多于**`long`，至少为8个字节。
...
- C标准对基本数据类型只规定了允许的最小取值范围. 

  16位机, short和int最小取值范围都是16位有符号取值范围.

  32位机, long的最小取值范围是32位有符号取值范围.

  对unsigned short 和unsigned int 最小取值范围是16位无符号数取值范围. 

  unsigned long最小取值范围是32位无符号数取值范围.	

  long long 和 unsigned long long 最小范围是64位的.

- 使用整型数, 优先考虑unsigned(如果变量取值不包含负值的话).

- 若一个数超出了int范围, 且在long范围, 使用long. 但在long占空间比int大的系统, long会减慢运算速度.

- long和int占空间一样的机器上, 应用long, 以便移植到16位.(使用移植类型)

- 编程时, 使用数据类型, 需要考虑到寄存器的位数.

## 浮点数类型

任何有小数点的数值，都会被编译器解释为浮点数。所谓“浮点数”就是使用 m * b<sup>e</sup> 的形式，存储一个数值，`m`是小数部分，`b`是基数（通常是`2`），`e`是指数部分。这种形式是精度和数值范围的一种结合，可以表示非常大或者非常小的数。

计算机如存储浮点型, 请查看 `IEEE754` 标准.
...
`C99 标准`添加了类型`_Bool`，表示布尔值。但是，这个类型其实只是整数类型的别名，还是使用`0`表示伪，`1`表示真，下面是一个示例。
...
- sizeof()和+=、*=一样是一个**复合运算符**, 由sizeof和()两个部分组成, 但是代表的是一个整体
- 所以sizeof不是一个函数, 是一个运算符, 该运算符的优先级是第二高的.
...
`sizeof`运算符的返回值，C 语言只规定是**`无符号整数`**(**注意运算的时候转换**)，并没有规定具体的类型，而是留给系统自己去决定，`sizeof`到底返回什么类型。不同的系统中，返回值的类型有可能是`unsigned int`，也有可能是`unsigned long`，甚至是`unsigned long long`，对应的`printf()`占位符分别是`%u`、`%lu`和`%llu`。这样不利于程序的可移植性。
...
自动转换又称隐式转换. 发生隐式转换的情况:

- 当算术或逻辑表达式中的操作数类型不相同.
- 赋值运算符两边类型不匹配.
- 函数调用过程中形参实参不匹配
- return语句中表达式类型与函数返回类型不匹配
...
- 空间占用小的数据, 转换位空间大的数据
  ...
  - 整数 : char--> short --> unsigned short --> int --> unsigned --> long --> unsigned long --> long long --> unsigned long long 
  - 整数等级低于浮点数(因为浮点数在内存中科学计数法, float可表示范围都大于unsigned long long)

- 转换规则

  - C99中, 可以将任何等级低于int或unsigned的类型转换到此类型, 运行结束后再转换. 如char
  - 如果两个数都没有浮点数, 首先将两个操作数整数提升
    - 如果两个数都是无符号型或有符号型, 则将等级低的类型, 转换成等级高的类型
    - 如果无符号数等级高于有符号数, 则将有符号数转换成无符号数的类型
    - 若有符号数类型可以表示无符号数类的取值范围, 将无符号数转换为有符号数类型
    - 否则, 两个操作数都转为与有符号数类型相对应的无符号数.
...
比如，`char`或`short`类型赋值给`int`类型，会自动提升为`int`。 高位用不到的会自动补零.
...
**char型转换成浮点型**, 先提升为整型值, 再添加小数位. 反过来浮点转字符, 先截断小数, 再截断高位.
...
变量的本质: 一块连续的存储单元(内存, 寄存器), 当变量存在寄存器中, 就没有地址了.

变量有名字, 用来表示存储单元(左值), 或存储单元的内容(右值).

数据对象没有名字, 是一种具有数据类型的连续存储单元.(堆内存中的空间.)