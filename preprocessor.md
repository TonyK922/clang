...
宏定义: #define #undef , 预定义宏, `__func__`, `__FUNCTION`, `__LINE__`
...
`#define`是最常见的预处理指令，用来将指定的词替换成另一个词。它的参数分成两个部分，第一个参数就是要被替换的部分，其余参数是替换后的内容。每条替换规则，称为一个**宏（macro）**。
...
**宏的名称不允许有空格**，而且**必须遵守 C 语言的变量命名规则**，只能使用字母、数字与下划线（`_`），且首字符不能是数字。

在预处理阶段, 编译器会全文查找#define, 不管#define定义在哪里, 从定义处开始全文件有效.

宏是原样替换，指定什么内容，就一模一样替换成什么内容。此过程称为 宏替换或宏展开.
...
**`#define`指令可以出现在源码文件的任何地方，从指令出现的地方到文件末尾都有效。不受作用域影响, 即使是在函数中定义宏, 此函数未被调用, 也开始从定义处生效.**习惯上，会将`#define`放在源码文件的头部。它的主要好处是，会使得程序的可读性更好，也更容易修改。
...
大工程, 跟别人一起协作开发. 那么模块的封装就很重要了.  头文件的作用也就重要了起来.

头文件中一般都有哪些内容:

函数声明,  构造数据类型声明, 宏定义, 常量的定义等等
...
形式一，文件名写在尖括号里面，表示该文件是系统提供的，通常是标准库的库文件，不需要写路径。因为编译器会到系统指定的安装目录里面，去寻找这些文件。这也是集成开发环境IDE的标准路径.
...
- 避免函数原型的重复声明, 利于程序模块化设计
- 避免宏定义多次重复声明
- 不会明显增加程序大小, 因为头文件的内容是编译器需要的信息, 而不是加到最终代码里的具体语句.
...
下面介绍 `#if #endif` `#ifdef` `#if defined` `#ifndef`
...
- `__FILE__`：当前文件名。
- `__LINE__`：当前行号。
- `__func__`：当前正在执行的函数名。该预定义宏必须在函数作用域使用。GCC预定义标识符
...
```c
#line 300 "newfilename" //改文件名 必须同时指定从300行处开始, 是newfilename
```
...
`#pragma`指令用来修改编译器属性。且 在源文件`声明的位置以下起作用`.
...
- 结构体成员的对齐模数, 是`#pragma pack(n)`指定值(亦默认值), 和该成员所占空间长度的`较小值`. 
	- 与`__attribute__((aligned(n)));` 不同的是, `#pragma` 取较小值, 8字节数据可以4字节方式对齐. 
	- 而attribute 指定的对齐, 只能比原字节大, 1字节数据可以2字节对齐, 4字节对齐, 8字节对齐, 而4字节数据只能4字节 8字节对齐.

  ```c
  #pragma pack(4)
  struct foo {
  	char a; // 对齐模数1, char占1字节, 指定4字节 
      double b; //类型8个字节, 指定值4个字节, 8>4 对齐模数4
      int c;   //对齐模数4
  };
  ```

- 结构体对齐模数, 是`#pragma pack(n)`指定值和结构体内`最大成员数据类型长度`的较小值.

  继续上面的代码, 结构体对齐模数就是4字节.

- 如果不写对齐模数, 那对齐模数默认跟CPU的数据线的位数一致. 

- C99有个新的操作符, `_Pragma("pack(n)")` 跟 `#pragma pack(n)`等价

## 注意点

一般使用全大写字母表示宏名, 可放在头文件;

宏定义可以嵌套使用;

使用宏定义多用括号, 以避免优先级导致的问题.

括号括住每个参数, 保证每个参数在定义表达式中能正确分组.

宏定义作用域: 定义开始, 到文件结束或者`#undef`处.

宏定义要单独占用一个逻辑行, 若定义字符太长, 可以用<KBD>\\</KBD> 来换行续接.