...
printf("%d\n", RED); // 0   #define RED 0
printf("%d\n", GREEN);  // 1 #define GREEN 1
printf("%d\n", BLUE);  // 2  #define BLUE 2
```
...
注意，Enum 内部的常量名，遵守标识符的命名规范，但是`通常都使用大写`。 //因为是常量
...
由于 Enum 的属性会自动声明为常量，所以有时候使用 Enum 的目的，不是为了自定义一种数据类型，而是**为了声明一组常量**。这时就可以使用下面这种写法，比较简单。
